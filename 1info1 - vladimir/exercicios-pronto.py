...
def tempo_para_percorrer_uma_distancia(distancia, velocidade):
    ...
    horas_gastas= distancia / velocidade
    return round(horas_gastas , 2)
...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    novo preço"""
    pd = preco_original - (preco_original * percentual_desconto / 100)
    return round(pd, 2)
...
def preco_aluguel_carro(dias, km):
    ...
    valor_ser_pago = (dias * 60) + (km  * 0.15)
    return valor_ser_pago
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    cigarros = cigarros_fumados_por_dia * (365 * anos_fumando)
    cigarros_dia= cigarros * (10) / 1440
    return round(cigarros_dia, 2)


def dois_elevado_a_um_milhao():
    """ Calcula dois elevado a um milhão, e retorna a quantidade de algarismos"""
...
def imprime_mes_por_extenso(data):
    ...
    data = ("{} de {} de {}".format(dia, meses[int(mes)], ano))
    return data
...
def media_temperaturas(temperaturas):
    ...
    """
    mt = sum(temperaturas) / len(temperaturas)
    return mt
